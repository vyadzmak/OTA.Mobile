...
export default class PrepareOrderScreen extends React.Component {
  ...
  change_address(_address) {
    ...
    for (i = 0; i < this.state.addresses.length; i++) {
      if (this.state.addresses[i].address == _address) {
        ...
        this.setState({
          order_params: { ...this.state.order_params, client_address_id: id }
        });
  ...
  confirm_order() {
    if (this.state.confirm_order == true) {
      this.setState({
        confirm_order: false
      });
    } else if (this.state.confirm_order == false) {
      this.setState({
        confirm_order: true
      });
    }
      ...
      return (
        <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
          <Container>
            <Content>
              <Form>
                <Text>Выберите адрес заказа</Text>
                <Picker
                  selectedValue={this.state.address}
                  onValueChange={(itemValue, itemIndex) =>
                    this.change_address(itemValue)
                  }>
                ...
                <Text style={styles.nameTextStyle}>Подтвердить заказ</Text>
                <Switch
                  value={this.state.confirm_order}
                  onValueChange={() => this.confirm_order()}
                />

                <Button
                  block
                  success
                  onPress={() => this.prepare_order()}
                  style={{ marginTop: 10 }}>
                  <Text>ОТПРАВИТЬ</Text>
                </Button>
              </Form>
