...
export default class TabProfileClient extends React.Component {
    ...
    try {
      ...
      response = postRequest(update_route, data).then(response => {
        ...
        if (status == 400) {
          ...
          //this.cart = response
          this.state.userProfile.user_data.client_data.client_addresses.push(
            response
          );
          ...
          this.setState({
            c_address: { ...this.state.c_address, address: "" }
          });
  ...
  _toggleModal(isCancel) {
    if (isCancel == true && this.state.isModalVisible == true) {
      ...
      this.setState({
        c_address: { ...this.state.c_address, tobacco_alcohol_license: false }
      });
    ...
    if (this.state.isModalVisible == true) {
      if (
        this.state.c_address.name == "" ||
        this.state.c_address.name.length < 3
      ) {
      ...
      if (
        this.state.c_address.address == "" ||
        this.state.c_address.address.length < 3
      ) {
  ...
  loadCities(city_id) {
    params = [
      ...
      { name: "area_id", value: city_id }
    ];
    _route = "/cityCatalogByArea";
    response = getWithParams(_route, params).then(response => {
      ...
      this.setState({ cities: [], city: {} }, function() {
        this.setState({ cities: this.cities, city: this.city }, function() {
          this.setState({
            c_address: { ...this.state.c_address, city_id: this.city.id }
          });
        });
      });
    });
  ...
  loadAreas() {
    response = getWithSlashParams("/areaCatalog").then(response => {
      ...
      if (response.message == undefined) {
        ...
        this.setState({
          client_data: {
            ...
            client_registration_number: this.state.userProfile.user_data
              .client_data.registration_number,
            client_phone_number: this.state.userProfile.user_data.client_data
              .client_info.phone_number,
            client_email: this.state.userProfile.user_data.client_data
              .client_info.email,
            client_main_info: this.state.userProfile.user_data.client_data
              .client_info.main_info,
            client_additional_info: this.state.userProfile.user_data.client_data
