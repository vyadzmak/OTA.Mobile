    ...
    try {
      ...
      response = getWithParams(r, params).then(response => {
        ...
        if (response.cart_id != undefined) {
          ...
          Toast.show("Продукты из заказа были добавлены в корзину");
        } else {
          Toast.show("Ошибка повторения заказа");
        }
      });
    } catch (err) {}
  }
      ...
      return (
        <Container>
          <Header style={styles.headerStyle}>
            <Text style={styles.headerText}>ИСТОРИЯ ЗАКАЗОВ</Text>
          </Header>
          <Content>
            <List
                ...
                <ListItem>
                  <TouchableOpacity
                    ...
                    <Body>
                      ...
                      <Text note>
                        Дата заказа: {item.display_creation_date}
                      </Text>
                      <Text note>
                        Адрес: {item.client_address_data.address}
                      </Text>
                      <Text note>
                        Сумма: {item.total_amount}{" "}
                        {item.currency_data.display_value}
                      </Text>
                  ...
                  <Button
                    block
                    success
                    onPress={() => this.repeat_order(item.id)}>
                    <Text>ПОВТОРИТЬ</Text>
                  </Button>
                </ListItem>
              )}>
