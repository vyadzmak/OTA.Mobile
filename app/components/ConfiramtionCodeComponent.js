...
export default class ConfirmationCodeForm extends React.Component {
  constructor(props){
   ...
   get_confirmation_code=()=>{
      AsyncStorage.getItem('confirmation_code').then((value) => {
        this.setState({temp_code: value});
    })
    .then(res => {
      Alert.alert('Код подтверждения', 'Ваш код подтверждения: '+this.state.temp_code)
    });
    }
    ...
    code_confirmation(){
      ...
      params =[{"name":"user_id","value":this.state.temp_user_id},{"name":"code","value":this.state.code}]
        ...
        response=> {
          ...
          if (response.message!=undefined){
            if (response.message=='Error'){
              Alert.alert('Ошибка подтверждения кода', 'Повторите код еще раз! Ваш код подтвержения: '+this.state.temp_code)
              }
            }

          if (response.status_code!=undefined){
            if (response.status_code==200){
                Alert.alert('Успех подтверждения кода', 
                'Код успешно подтвержден! Войдите в систему со своим логином и паролем. Приятной работы!',
                [
                  
                  {text: 'ОК', onPress: () => {this.redirect_to_login() }},
                ],
                { cancelable: false }
              )
      ...
      <KeyboardAwareScrollView>
        <View >
          ...
          <TextInput style={styles.inputBox} 
           ...
           <TouchableOpacity style={styles.button} onPress={()=>this.code_confirmation()}>
